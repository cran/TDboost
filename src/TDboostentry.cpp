...
)
{
    ...
    // allocate the TDboost
    pTDboost = new CTDboost();
    if(pTDboost==NULL)
    {
        hr = TDboost_OUTOFMEMORY;
        goto Error;
    ...
    // initialize the TDboost
    hr = pTDboost->Initialize(pData,
                          pDist,
                          REAL(rdShrinkage)[0], 
                          cTrain, 
                          REAL(rdBagFraction)[0],
    ...
    for(iT=0; iT<cTrees; iT++)
    {
        ...
        hr = TDboost_transfer_to_R(pTDboost,
                               vecSplitCodes,
                               INTEGER(riSplitVar),
                               REAL(rdSplitPoint),
                               INTEGER(riLeftNode),
    ...
    for(i=0; i<(int)vecSplitCodes.size(); i++)
    {
        ...
        hr = TDboost_transfer_catsplits_to_R(i,
                                         vecSplitCodes,
                                         INTEGER(rSplitCode));
    }
...
)
{
   ...
   for(iPredIteration=0; iPredIteration<LENGTH(rcTrees); iPredIteration++)
   {
      ...
      while(iTree<cTrees)
      {
         ...
         // these relate to columns returned by pretty.TDboost.tree()
