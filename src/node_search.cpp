...
TDboostRESULT CNodeSearch::Initialize
(
...
TDboostRESULT CNodeSearch::IncorporateObs
(
    double dX,
    double dZ,
    double dW,
...
)
{
    ...
    else if(cCurrentVarClasses == 0)   // variable is continuous
    {
        if(dLastXValue > dX)
        {
            error("Observations are not in order. TDboost() was unable to build an index for the design matrix. Could be a bug in TDboost or an unusual data type in data.\n");
            hr = TDboost_FAIL;
            goto Error;
        }
...
TDboostRESULT CNodeSearch::ResetForNewVar
(
...
TDboostRESULT CNodeSearch::WrapUpCurrentVariable()
{
    TDboostRESULT hr = TDboost_OK;
...
TDboostRESULT CNodeSearch::EvaluateCategoricalSplit()
{
    TDboostRESULT hr = TDboost_OK;
    long i=0;
    long j=0;
...
TDboostRESULT CNodeSearch::SetupNewNodes
(
